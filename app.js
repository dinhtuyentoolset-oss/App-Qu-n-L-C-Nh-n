...
async function hashPassword(pw){
  if(!crypto?.subtle){
    throw new Error('Trình duyệt chặn mã hóa mật khẩu. Mở app qua HTTPS (GitHub Pages), không mở file HTML trực tiếp từ ổ đĩa.');
  }
...
async function doLogin(){
  ...
  console.log('DEBUG login:', username, pw ? '***' : 'EMPTY', 'gsUrl:', getGsUrl() ? 'OK' : 'MISSING');
  ...
  try{
    ...
    console.log('DEBUG login result:', result.status, result.error || 'ok');
    if(result.status!=='ok'){
      let err=result.error||'Đăng nhập thất bại.';
      if(/mật khẩu|password/i.test(err)){
        err+=' Nếu vừa tách file: deploy lại Apps Script (New version) và đăng nhập lại.';
      }
      showAuthError(err);
      hideAuthLoading();
      return;
