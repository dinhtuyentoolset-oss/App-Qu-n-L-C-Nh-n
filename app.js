...
function getGsUrl(){
  const url = (typeof GS_WEBAPP_URL !== 'undefined' ? GS_WEBAPP_URL : window.GS_WEBAPP_URL) || '';
  return String(url).trim();
}

function getAppVersion(){
  if(typeof APP_VERSION !== 'undefined') return String(APP_VERSION).trim();
  if(window.APP_VERSION) return String(window.APP_VERSION).trim();
  return 'v1';
}
...
async function hashPassword(pw){
  if(location.protocol==='file:'){
    throw new Error('Không mở app bằng file://. Chạy qua http(s): Live Server, GitHub Pages, hoặc: python -m http.server trong thư mục app.');
  }
  if(!crypto?.subtle){
    throw new Error('Trình duyệt chặn mã hóa mật khẩu. Mở app qua HTTPS hoặc localhost (http), không mở file HTML trực tiếp từ ổ đĩa.');
  }
...
function hideAuthLoading(){
  ...
  const regActive=document.getElementById('authTabRegister')?.classList.contains('active');
  const forgotEl=document.getElementById('authPanelForgot');
  const tab=regActive?'register':(forgotEl&&!forgotEl.classList.contains('hidden')?'forgot':'login');
...
function onAuthFormKeydown(e){
  ...
  const forgotPanel = document.getElementById('authPanelForgot');

  if(loginPanel && !loginPanel.classList.contains('hidden')){
    if(t.id === 'loginUsername' || t.id === 'loginPassword'){
      e.preventDefault();
      doLogin();
    }
    return;
  }
  if(registerPanel && !registerPanel.classList.contains('hidden')){
    if(t.id === 'regDisplayName' || t.id === 'regUsername' || t.id === 'regPassword' || t.id === 'regPassword2'){
      e.preventDefault();
      doRegister();
    }
    return;
  }
  if(forgotPanel && !forgotPanel.classList.contains('hidden')){
    if(t.id === 'forgotUsername'){
      e.preventDefault();
      doForgotPassword();
    }
  }
...
function setupAuthEnterKeyHandlers(){
  if(authEnterKeysBound) return;
  const bind = () => {
    if(authEnterKeysBound) return;
    const root = document.getElementById('authScreen');
    if(!root) return;
    root.addEventListener('keydown', onAuthFormKeydown);
    authEnterKeysBound = true;
  };
...
async function pullUserDataFromSheets(username, opts){
  ...
  migrateThemeIntoStateIfNeeded(data);
  localStorage.setItem(userDataKey(username), JSON.stringify(data));
  if(getActiveUsername()===username){
    applyTheme(themeFromState(data));
    cacheThemeLocal(data.theme);
  }
...
async function doLogin(){
  ...
  if(!getGsUrl()){ showAuthError('Không đọc được config.js hoặc thiếu GS_WEBAPP_URL. Kiểm tra 4 file cùng thư mục (index.html, config.js, app.js, styles.css).'); return; }
  ...
  try{
    ...
    if(result.status!=='ok'){
      showAuthError(result.error||'Đăng nhập thất bại.');
      hideAuthLoading();
      return;
    }
    sessionStorage.setItem(AUTH_SESSION_KEY, username);
    ...
    hideAuthLoading();
    showAuthError('Lỗi đăng nhập: '+e.message);
  }
}
async function doForgotPassword(){
  ...
  try{
    ...
    if(resultEl){
      resultEl.textContent='Tài khoản tồn tại. Mật khẩu được mã hóa — không lấy lại qua app. Liên hệ admin để reset, hoặc đăng ký username mới.';
...
function doLogout(){
  ...
  stopMultiDeviceSync();
  if(typeof stopTaskUpgradeTimers==='function') stopTaskUpgradeTimers();
...
// TABS
// ============================================================
const TAB_IDS = ['overview','finance','savings','tasks','journey','time','lifelog','projects','settings'];
...
function refreshCurrentTab(){
  if(currentTab==='overview' && typeof renderOverview==='function') renderOverview();
  ...
  else if(currentTab==='lifelog' && typeof renderLifeLog==='function') renderLifeLog();
  else if(currentTab==='projects' && typeof renderProjects==='function') renderProjects();
...
function renderList(id, arr, type){
  ...
  `).join('') || emptyStateHtml('📝','Chưa có mục nào','Bấm + Thêm để bắt đầu ghi nhận.');
}
...
function saveTodaySpend(){
  ...
  const spendInput=document.getElementById('spend-today-input');
  if(spendInput){
    spendInput.value='';
    spendInput.focus();
  }
...
function renderSavings(){
  if(typeof renderSavingsAlerts==='function') renderSavingsAlerts();
  const s = getState();
  const cfg = s.cfg;
  const sav = s.savings;
...
// TASKS
// ============================================================
const TASK_PRIORITY_LABELS = {low:'Thấp',medium:'Vừa',high:'Cao',urgent:'Gấp'};
const TASK_PRIORITY_BADGE = {low:'badge-purple',medium:'badge-blue',high:'badge-yellow',urgent:'badge-red'};
function normalizeTaskPriority(t){ return t.priority||'medium'; }
function taskSortScore(t, today){
  if(!t.done && t.due && t.due<today) return 0;
  if(!t.done && t.due===today) return 1;
  if(!t.done && normalizeTaskPriority(t)==='urgent') return 2;
  if(!t.done && normalizeTaskPriority(t)==='high') return 3;
  if(t.done) return 9;
  return 5;
}
function renderTasks(){
  const s = getState();
  let tasks = (s.tasks||[]).map(t=>({...t,priority:normalizeTaskPriority(t)}));
  ...
  filtered = [...filtered].sort((a,b)=>{
    if(typeof compareTasksByDeadline==='function'){
      const c=compareTasksByDeadline(a,b,today);
      if(c!==0) return c;
    }
    const sa=taskSortScore(a,today), sb=taskSortScore(b,today);
  ...
  const todayList=tasks.filter(t=>!t.done&&t.due===today);
  const urgentList=tasks.filter(t=>!t.done&&(t.priority==='urgent'||t.priority==='high'));
  let taskPanels='';
  if(taskFilter==='all'&&(todayList.length||urgentList.length)){
    if(todayList.length){
      taskPanels+=`<div class="task-highlight-card"><div class="task-highlight-title">📌 Việc hôm nay</div>${todayList.map(t=>{
        const ri=tasks.indexOf(t);
        return `<div class="task-mini" onclick="editTask(${ri})">${esc(t.name)}</div>`;
      }).join('')}</div>`;
    }
    if(urgentList.length){
      taskPanels+=`<div class="task-highlight-card urgent"><div class="task-highlight-title">🔥 Việc gấp / ưu tiên cao</div>${urgentList.slice(0,5).map(t=>{
        const ri=tasks.indexOf(t);
        return `<div class="task-mini" onclick="editTask(${ri})">${esc(t.name)} <span class="badge ${TASK_PRIORITY_BADGE[t.priority]}">${TASK_PRIORITY_LABELS[t.priority]}</span></div>`;
      }).join('')}</div>`;
    }
  }

  document.getElementById('task-list').innerHTML = taskPanels + (filtered.map((t,i)=>{
    ...
    const pri=normalizeTaskPriority(t);
    const dueLabel = t.due
      ? `${isOverdue?'⚠️ Quá hạn · ':''}📅 ${esc(t.due)}${t.dueTime?` 🕐 ${esc(t.dueTime)}`:''}`
      : '';
    return `
    <div class="task-item ${t.done?'done':''} ${isOverdue?'task-overdue':''}" data-task-idx="${realIdx}">
      ...
      <div class="task-body">
        ...
        <div class="task-meta">
          ...
          <span class="badge ${TASK_PRIORITY_BADGE[pri]}">${TASK_PRIORITY_LABELS[pri]}</span>
          ${typeof getTaskUrgencyBadgeHtml==='function'?getTaskUrgencyBadgeHtml(t):''}
  ...
  }).join('') || emptyStateHtml('📋','Chưa có công việc','Bấm + Thêm việc để bắt đầu quản lý công việc.'));
  if(typeof applyUpgradeUIClasses==='function') applyUpgradeUIClasses();
}
...
function openAddTask(){
  ...
  const tp=document.getElementById('taskPriority');
  if(tp) tp.value='medium';
...
function editTask(idx){
  ...
  const tp=document.getElementById('taskPriority');
  if(tp) tp.value=normalizeTaskPriority(t);
...
function saveTask(){
  ...
  if(idx!==''){
    ...
    task.priority = task.priority || normalizeTaskPriority(s.tasks[idx]);
    if(typeof ensureTaskId==='function') ensureTaskId(s.tasks[idx]);
    task.id = s.tasks[idx].id;
    const oldDue = s.tasks[idx].due + (s.tasks[idx].dueTime||'');
    const newDue = task.due + (task.dueTime||'');
    if(oldDue !== newDue && typeof clearTaskWarn==='function') clearTaskWarn(task.id);
    s.tasks[parseInt(idx)] = task;
  } else {
    if(typeof ensureTaskId==='function') ensureTaskId(task);
    s.tasks.push(task);
  }
  saveState(s);
...
function getJourneyStatus(j){
  ...
  if(st.pct>=100) return {label:'Hoàn thành',cls:'badge-green'};
  ...
  if(now>=start&&now<=end&&keys.includes(todayKey)&&!(j.completedDays||{})[todayKey]){
    return {label:'Trễ nhịp',cls:'badge-red'};
  }
  if(now>end&&st.pct<100) return {label:'Trễ nhịp',cls:'badge-red'};
  return {label:'Đang chạy',cls:'badge-blue'};
}
...
function renderJourneys(){
  ...
  if(!journeys.length){
    list.innerHTML=emptyStateHtml('🏆','Chưa có hành trình nào','Tạo hành trình 7, 21, 30 ngày... và tick từng ngày hoàn thành!');
    return;
  }
  list.innerHTML=journeys.map(j=>{
    const st=getJourneyStats(j);
    const status=getJourneyStatus(j);
    const end=getJourneyEnd(j);
    ...
    <div class="card journey-card">
      <div class="card-header">
        <div>
          <div class="card-title">🏆 ${esc(j.name)} <span class="badge ${status.cls}">${status.label}</span></div>
...
function updatePomoUI(){
  ...
  const weekEl=document.getElementById('pomo-week-count');
  const monthEl=document.getElementById('pomo-month-count');
  if(weekEl) weekEl.textContent = typeof getPomoWeekCount==='function'?getPomoWeekCount():0;
  if(monthEl) monthEl.textContent = typeof getPomoMonthCount==='function'?getPomoMonthCount():0;
}
...
function toggleSettingsAdvanced(){
  ...
  if(btn) btn.textContent=open?'▲ Thu gọn cài đặt nâng cao':'⚙️ Cài đặt nâng cao';
  if(open && typeof updateSystemStatus==='function') updateSystemStatus();
}
...
function exportData(){
  const data = getState();
  const u=getActiveUsername()||'user';
  const blob = new Blob([JSON.stringify(data,null,2)],{type:'application/json'});
  const a = document.createElement('a');
  a.href = URL.createObjectURL(blob);
  a.download = `app-ca-nhan-${u}-${new Date().toISOString().slice(0,10)}.json`;
  a.click();
...
function importData(evt){
  ...
  if(!file) return;
  if(!confirm('Nhập backup sẽ gộp dữ liệu vào tài khoản hiện tại. Tiếp tục?')){ evt.target.value=''; return; }
  ...
  reader.onload = e=>{
    try{
      ...
      const cur = getState();
      const merged = mergeData(cur, data);
      migrateThemeIntoStateIfNeeded(merged);
...
async function syncActiveVersionToSheet(){
  ...
  let lastErr = '';
  try{
    data = await gsApiPost({ action:'setActiveVersion', version: getAppVersion() });
  }catch(e){
    lastErr = e.message||String(e);
    try{
      data = await gsApiGet({ action:'setActiveVersion', version: getAppVersion() });
    }catch(e2){
  ...
  if(data.status==='ok'){
    sheetActiveVersion = String(data.activeVersion||getAppVersion()).trim();
...
function updateVersionDisplay(){
  ...
  if(!el) return;
  if(!getGsUrl()){
    el.innerHTML = `<div>Phiên bản file này: <strong style="color:var(--accent);">${esc(getAppVersion())}</strong></div>
  ...
  el.innerHTML = `
    <div>File HTML này: <strong style="color:var(--accent);font-family:var(--mono);">${esc(getAppVersion())}</strong></div>
  ...
  if(result.ok){
    ...
    if(hint.includes('setActiveVersion') || hint.includes('Unknown')){
      alert(
        ...
        'Tạm thời: gõ tay '+getAppVersion()+' vào tab Config, ô B4.'
      );
    }
  }
}
...
function updateTelegramDiagnostics(){
  ...
  el.innerHTML = `
    ...
    <div>${verOk?'✅':'❌'} Đã chốt bản <code>${esc(getAppVersion())}</code> (Config B4 = ${esc(sheetActiveVersion||'—')})</div>
...
async function sendTelegram(text, opts){
  ...
  if(!opts?.skipVersionCheck && !isTelegramVersionActive()){
    if(opts?.showError){
      showToast(`Telegram tắt: file ${getAppVersion()} ≠ B4 (${sheetActiveVersion||'—'}). Chốt lại bản này.`,'error');
      updateVersionDisplay();
    }
    return false;
  }
...
async function testTelegram(){
  ...
  if(!isTelegramVersionActive()){
    showToast(`Chưa chốt bản ${getAppVersion()} (B4 hiện: ${sheetActiveVersion||'—'}).`,'error');
    updateVersionDisplay();
    return;
  }
  const s = getState();
  const name = s.cfg.name||'Bạn';
  const ok = await sendTelegram(`👋 Test từ App Cá Nhân (${getAppVersion()})\nXin chào ${name}! Nếu thấy tin này là Telegram OK ✅`, { showError: true });
  if(ok){
  ...
  [...(local||[]), ...(remote||[])].forEach(t=>{
    ...
    if(!map.has(key)) map.set(key, {...t});
      ...
      map.set(key, {
        ...
        priority: b.priority || a.priority || 'medium'
      });
    }
  });
  return Array.from(map.values());
...
function mergeData(local, remote){
  ...
  const lifeLogMap=new Map();
  [...(local.lifeLog||[]),...(remote.lifeLog||[])].forEach(l=>{
    if(!l||!l.id) return;
    lifeLogMap.set(l.id, lifeLogMap.has(l.id)?{...lifeLogMap.get(l.id),...l}:l);
  });
  const projectsMap=new Map();
  [...(local.projects||[]),...(remote.projects||[])].forEach(p=>{
    if(!p||!p.id) return;
    const prev=projectsMap.get(p.id);
    projectsMap.set(p.id, prev?{...prev,...p,tasks:[...(prev.tasks||[]),...(p.tasks||[])].filter((t,i,a)=>a.findIndex(x=>x.id===t.id)===i)}:p);
  });
  return {
    months,
    ...
    lifeLog: Array.from(lifeLogMap.values()).sort((a,b)=>(b.date||'').localeCompare(a.date||'')),
    ...
    theme: mergeThemeState(local, remote),
...
// ============================================================
// THEME — lưu trong state.theme (đồng bộ Sheets theo tài khoản)
// ============================================================
...
function themeFromState(s){
  ...
  return {
    preset:t.selectedTheme??t.preset??'dark',
    accent:t.accent??DEFAULT_ACCENT,
    bgImage:t.wallpaper??t.bgImage??null,
    bgOverlay:t.overlay??t.bgOverlay??40,
    font:t.font??'default',
    radius:t.radius??16,
...
function themeToStateShape(t){
  const m={...DEFAULT_THEME,...t};
  return {
    selectedTheme:m.preset,
    accent:m.accent,
    font:m.font,
    radius:m.radius,
    overlay:m.bgOverlay,
...
function hasThemeInState(s){
  const t=s?.theme;
  return !!(t&&typeof t==='object'&&(t.selectedTheme||t.preset||t.accent||t.wallpaper||t.bgImage||t.avatar||t.font));
}

function loadLegacyThemeLocalOnly(){
  const u=getActiveUsername();
  if(!u) return null;
  try{
    const r=localStorage.getItem(userThemeKey(u));
    return r?JSON.parse(r):null;
  }catch(e){ return null; }
}

function cacheThemeLocal(themeState){
  const u=getActiveUsername();
  if(!u||!themeState) return;
  try{
    localStorage.setItem(userThemeKey(u),JSON.stringify(themeFromState({theme:themeState})));
  }catch(e){ /* quota */ }
}

function migrateThemeIntoStateIfNeeded(s){
  if(hasThemeInState(s)) return s;
  const legacy=loadLegacyThemeLocalOnly();
  s.theme=themeToStateShape(legacy?{...DEFAULT_THEME,...legacy}:DEFAULT_THEME);
  const u=getActiveUsername();
  if(u) localStorage.setItem('lifeos_theme_migrated_'+u,'1');
  return s;
}

/** Trả true nếu vừa thêm theme vào state (cần lưu + sync một lần) */
function ensureThemeInState(s){
  if(hasThemeInState(s)){
    cacheThemeLocal(s.theme);
    return false;
  }
  migrateThemeIntoStateIfNeeded(s);
  cacheThemeLocal(s.theme);
  return true;
}

function mergeThemeState(local, remote){
  const lt=local?.theme;
  const rt=remote?.theme;
  if(!hasThemeInState({theme:lt})&&!hasThemeInState({theme:rt})) return themeToStateShape(DEFAULT_THEME);
  if(!hasThemeInState({theme:lt})) return rt;
  if(!hasThemeInState({theme:rt})) return lt;
  return (remote.stateUpdatedAt||'')>=(local.stateUpdatedAt||'')?rt:lt;
}

function saveTheme(t){
  const s=getState();
  s.theme=themeToStateShape(t);
  saveState(s);
  cacheThemeLocal(s.theme);
}

function getTheme(){
  return {...DEFAULT_THEME,...themeFromState(getState())};
}
const THEME_PRESETS = {
...
async function init(){
  ...
  updateBackendInfo();
  updateTelegramDiagnostics();
  if(typeof initAppUpgrades==='function') initAppUpgrades();
}
...
if(!getGsUrl()){
  document.addEventListener('DOMContentLoaded', ()=>{
    showAuthError('Không tải được config.js hoặc thiếu GS_WEBAPP_URL. Đặt index.html, config.js, app.js, styles.css cùng một thư mục.');
  });
}
if(location.protocol==='file:'){
  document.addEventListener('DOMContentLoaded', ()=>{
    showAuthError('Không mở app bằng file://. Dùng Live Server, GitHub Pages, hoặc chạy: python -m http.server trong thư mục chứa index.html.');
  });
}
setupAuthEnterKeyHandlers();
initAuth();
