...
function renderProjects(){
  ...
  const listHtml=list.length?list.map(p=>{
    ...
    const phasePct=typeof calcProjectPhaseProgress==='function'?calcProjectPhaseProgress(p.phases):null;
    const displayPct=phasePct!==null?phasePct:(p.progress||0);
    const barCls=typeof getProjectProgressBarClass==='function'?getProjectProgressBarClass(displayPct):'';
    const phasesHtml=typeof renderProjectPhasesInline==='function'?renderProjectPhasesInline(p):'';
    return `
    <div class="project-item card upgrade-glass ${near?'project-warn':''}">
      ...
      <div class="project-meta">
        ...
        <span class="upgrade-mono-num">📊 ${displayPct}%</span>
        <span>✓ ${doneT}/${tasks.length} việc nhỏ</span>
      </div>
      <div class="prog-bar-sm upgrade-phase-bar"><div class="prog-fill-sm ${barCls} upgrade-phase-fill" style="width:${Math.min(100,displayPct)}%"></div></div>
      ${phasesHtml}
...
function openProjectModal(editId){
  ...
  if(editId){
    ...
    if(typeof renderProjectPhasesEditor==='function') renderProjectPhasesEditor(p.phases||[]);
    ...
    if(typeof renderProjectPhasesEditor==='function') renderProjectPhasesEditor([]);
...
function saveProject(){
  ...
  const progressManual=Math.min(100,Math.max(0,parseInt(document.getElementById('projectProgress').value,10)||0));
  ...
  const phases=typeof collectProjectPhasesFromEditor==='function'?collectProjectPhasesFromEditor():[];
  const phaseProgress=typeof calcProjectPhaseProgress==='function'?calcProjectPhaseProgress(phases):null;
  const progress=phaseProgress!==null?phaseProgress:progressManual;
  const s=getState();
  ...
  if(editId){
    ...
    const oldPhaseMap=new Map((old?.phases||[]).map(ph=>[ph.id,ph]));
    tasks=taskLines.map((n,i)=>{
      const prev=oldMap.get(n);
      return prev||{id:'pt_'+Date.now()+i,name:n,done:false};
    });
    phases.forEach((ph,i)=>{
      if(!ph.id) ph.id='ph_'+Date.now()+i;
      const prev=oldPhaseMap.get(ph.id);
      if(prev) ph.done=ph.done!==undefined?ph.done:prev.done;
    });
  }else{
    tasks=taskLines.map((n,i)=>({id:'pt_'+Date.now()+i,name:n,done:false}));
    phases.forEach((ph,i)=>{ if(!ph.id) ph.id='ph_'+Date.now()+i; });
  }
  const payload={name,status,priority,startDate,deadline,progress,note,tasks,phases,createdAt:new Date().toISOString()};
  if(editId){
