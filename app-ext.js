...
function renderMonthlyDashboard(){
  ...
  if(prev.income||prev.spent){
    compareHtml=`<div class="fin-compare">
      <div class="fin-compare-title">So với tháng trước</div>
      <div class="fin-compare-grid">
        <div><span class="lbl">Thu nhập</span><span class="val">${formatDelta(md.income-prev.income)}</span></div>
        <div><span class="lbl">Tổng chi</span><span class="val">${formatDelta(md.spent-prev.spent)}</span></div>
        <div><span class="lbl">Còn lại</span><span class="val">${formatDelta(md.remain-prev.remain)}</span></div>
      </div>
    </div>`;
  }else{
    compareHtml='<p class="fin-compare-empty">Chưa có dữ liệu tháng trước để so sánh.</p>';
  }

  const top3Html=md.top3.length?md.top3.map((e,i)=>`<div class="fin-top-item"><span>${i+1}. ${esc(e.name)}</span><strong>${fmtFull(e.amount)}</strong></div>`).join('')
    :'<p class="fin-compare-empty">Chưa có khoản chi.</p>';

  el.innerHTML=`
  <div class="fin-dash card">
    <div class="fin-dash-header">
      <div><div class="card-title">📊 Tổng quan tháng ${monthLabel}</div></div>
      <button type="button" class="btn btn-ghost btn-sm" onclick="renderFinance()">🔄 Làm mới</button>
    </div>
    <div class="fin-dash-stats">
      <div class="fin-stat"><div class="lbl">Tổng thu nhập</div><div class="val income">${fmtFull(md.income)}</div></div>
      <div class="fin-stat"><div class="lbl">Tổng đã chi</div><div class="val expense">${fmtFull(md.spent)}</div></div>
      <div class="fin-stat"><div class="lbl">Còn lại</div><div class="val ${md.remain>=0?'ok':'danger'}">${fmtFull(md.remain)}</div></div>
      <div class="fin-stat"><div class="lbl">Tỷ lệ tiết kiệm</div><div class="val">${md.income?Math.round(md.savingRate)+'%':'—'}</div></div>
      <div class="fin-stat"><div class="lbl">Chi cố định / Thu</div><div class="val">${md.income?Math.round(md.fixedRate)+'%':'—'}</div></div>
    </div>
    <div class="fin-progress-wrap">
      <div class="fin-progress-lbl"><span>Chi tiêu / Thu nhập</span><span>${md.income?Math.round(md.spentRate)+'%':'—'}</span></div>
      <div class="fin-progress"><div class="fin-progress-fill ${rateCls}" style="width:${md.income?Math.min(100,md.spentRate):0}%"></div></div>
    </div>
    <div class="fin-top3"><div class="section-title">3 khoản chi lớn nhất</div>${top3Html}</div>
    <div class="fin-insights">${insights.map(l=>`<p>💡 ${esc(l)}</p>`).join('')}</div>
    <div class="fin-alert ${rateCls}">${md.income?(md.spentRate>=90?'⚠️ Chi tiêu gần hoặc vượt thu nhập!':md.spentRate>=70?'⚡ Theo dõi chi tiêu — đang ở mức cảnh báo.':'✅ Tài chính tháng đang ổn.'):'ℹ️ Nhập thu nhập để xem cảnh báo.'}</div>
    ${compareHtml}
...
function renderYearDashboard(){
  ...
  if(!ys){
    el.innerHTML=`<div class="fin-year card"><div class="card-title">📅 Tổng quan năm ${currentYear}</div><p class="fin-compare-empty">Chưa có dữ liệu năm nay.</p></div>`;
    return;
  }
  ...
  <div class="fin-year card">
    <div class="card-title">📅 Tổng quan năm ${currentYear}</div>
    <div class="fin-dash-stats compact">
      <div class="fin-stat"><div class="lbl">Thu năm</div><div class="val">${fmtFull(ys.income)}</div></div>
      <div class="fin-stat"><div class="lbl">Chi năm</div><div class="val">${fmtFull(ys.spent)}</div></div>
      <div class="fin-stat"><div class="lbl">Tích lũy</div><div class="val">${fmtFull(ys.saving)}</div></div>
      <div class="fin-stat"><div class="lbl">Còn lại</div><div class="val">${fmtFull(ys.remain)}</div></div>
    </div>
    <p class="fin-year-note">Tháng chi nhiều nhất: <strong>${maxM}</strong> · Tháng tiết kiệm tốt: <strong>${bestM}</strong></p>
  </div>`;
}
...
function renderOverview(){
  ...
  const recentLogs=(s.lifeLog||[]).slice().sort((a,b)=>(b.date||'').localeCompare(a.date||'')).slice(0,3);
  const urgentProjects=(s.projects||[]).filter(p=>p.status==='active'&&(p.priority==='urgent'||p.priority==='high')).slice(0,3);

  const alertHtml=alerts.length?alerts.map(a=>`<div class="ov-alert-item" onclick="switchTab('${a.tab}')">${esc(a.t)} →</div>`).join('')
    :'<p class="ov-muted">Không có cảnh báo quan trọng.</p>';

  const logHtml=recentLogs.length?recentLogs.map(l=>`
    <div class="ov-log-item" onclick="switchTab('lifelog')">
      <span>${MOOD_EMOJI[l.mood]||'📝'} ${esc(l.title||'Nhật ký')}</span>
      <span class="ov-muted">${esc(l.date)}</span>
    </div>`).join(''):'<p class="ov-muted">Chưa có nhật ký.</p>';

  el.innerHTML=`
  <div class="ov-hero card">
    <div class="ov-hero-title" id="ov-greeting">—</div>
    <div class="ov-hero-sub">Tổng quan hôm nay · ${new Date().toLocaleDateString('vi-VN')}</div>
  </div>
  <div class="dash-grid">
    <div class="dash-card clickable" onclick="switchTab('finance')">
      <div class="dash-card-title">💰 Tài chính tháng này</div>
      <div class="dash-metric">Còn lại <strong class="${md.remain>=0?'ok':'danger'}">${fmtFull(md.remain)}</strong></div>
      <div class="dash-sub">Thu ${fmtNum(md.income)} · Chi ${fmtNum(md.spent)} · Tiết kiệm ${md.income?Math.round(md.savingRate)+'%':'—'}</div>
    </div>
    <div class="dash-card clickable" onclick="switchTab('tasks')">
      <div class="dash-card-title">📋 Công việc hôm nay</div>
      <div class="dash-metric">${todayTasks.length} việc · <span class="danger">${overdueTasks.length} quá hạn</span></div>
      <div class="dash-sub">Tổng ${tasks.length} việc trong danh sách</div>
    </div>
    <div class="dash-card clickable" onclick="switchTab('savings')">
      <div class="dash-card-title">🎯 Tiết kiệm</div>
      <div class="dash-metric">${esc(goalName)} · ${savPct}%</div>
      <div class="dash-sub">Mục tiêu ${goal?fmtFull(goal):'—'}</div>
    </div>
    <div class="dash-card clickable" onclick="switchTab('journey')">
      <div class="dash-card-title">🏆 Hành trình</div>
      <div class="dash-metric">${activeJourneys.length} đang chạy</div>
      <div class="dash-sub">${activeJourneys[0]?esc(activeJourneys[0].name):'Chưa có hành trình'}</div>
    </div>
    <div class="dash-card clickable" onclick="switchTab('time')">
      <div class="dash-card-title">⏰ Thời gian tập trung</div>
      <div class="dash-metric">🍅 ${pomoToday} hôm nay</div>
      <div class="dash-sub">Tuần ${getPomoWeekCount()} · Tháng ${getPomoMonthCount()}</div>
    </div>
    <div class="dash-card clickable" onclick="switchTab('lifelog')">
      <div class="dash-card-title">📔 Nhật ký gần đây</div>
      ${logHtml}
    </div>
    <div class="dash-card clickable wide" onclick="switchTab('settings')">
      <div class="dash-card-title">⚡ Hôm nay cần chú ý</div>
      ${alertHtml}
    </div>
    ${urgentProjects.length?`<div class="dash-card clickable" onclick="switchTab('projects')">
      <div class="dash-card-title">📁 Dự án quan trọng</div>
      ${urgentProjects.map(p=>`<div class="ov-log-item">${esc(p.name)} <span class="badge badge-red">${esc(p.priority)}</span></div>`).join('')}
    </div>`:''}
  </div>`;
  const gt=document.getElementById('ov-greeting');
  if(gt) gt.textContent=document.getElementById('greetingTitle')?.textContent||'Xin chào!';
}
